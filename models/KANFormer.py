...
class KANBlock(nn.Module):
    def __init__(self,input_dim, output_dim,image_size):
        ...
        self.kan_layer_01 = KANLinear(input_dim,input_dim)
        self.kan_layer_02 = KANLinear(input_dim,input_dim)

    def forward(self, x):
        B,C,H,W = x.shape
        shorcut = x
        x = rearrange(x, 'b c h w -> b (h w) c')
        x = self.kan_layer_01(x)
        x = self.kan_layer_02(x)
...
#     def __init__(self,HSI_bands=31,MSI_bands=3,hidden_dim=256,scale=4,depth=4,image_size=64):
#         super(Fusion, self).__init__()
#         self.conv = nn.Conv2d(HSI_bands+MSI_bands, hidden_dim, 3,1,1)
#         self.scale = scale

#     def forward(self, LRHSI, HRMSI):
#         up_LRHSI = F.interpolate(LRHSI, scale_factor=self.scale, mode='bicubic', align_corners=True)
#         feats = torch.cat([up_LRHSI, HRMSI], dim=1)
#         feats = F.relu(self.conv(feats))
#         return feats
...
class KANFormer(nn.Module):
    def __init__(self,HSI_bands=31,MSI_bands=3,hidden_dim=256,scale=4,depth=4,image_size=64):
        ...
        self.layers = nn.ModuleList([KANBlock(hidden_dim, hidden_dim,image_size=image_size) \
